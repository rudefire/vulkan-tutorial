...
private:
	...
	void setupDebugCallback()
	{
		...
		if (VK_SUCCESS != CreateDebugReportCallbackEXT(instance, &createInfo, nullptr, &callback))
	...
	//Used to specify a buffer resource to bind a uniform buffer descriptor to
	...
	void createDescriptorSet()
	{
		...
		if (VK_SUCCESS != vkAllocateDescriptorSets(device, &allocInfo, &descriptorSet))
	...
	{
		//might be a good idea to allocate a command pool for these kinds of short
		//lived buffers in a full renderer
	...
	void createCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); i++)
		{
			...
			VkClearValue clearColor = { .5f, .5f, .5f, 1.f };
	...
	void updateUniformBuffer()
	{
		...
		ubo.proj[1][1] *= -1; //this puts the y coord into the correct format for Vulkan
